...
from core.utils import save_image, video_ref, interpolate, slide
...
class Predictor(cog.Predictor):
    ...
    @cog.input("model", type=str, options=["celeb", "animal"], default="animal",
               help="use celeb (celeba_hq) or animal (afhq) model")
    @cog.input("src", type=Path, help="source facial image (decides pose for animals or identity for humans)")
    @cog.input("ref", type=Path, help="reference facial image (decides breed/style for animals or gender/style for humans)")
    def predict(self, src, ref, model):
        ...
        self.args.checkpoint_dir = 'expr/checkpoints/afhq' if model == 'animal' else 'expr/checkpoints/celeba_hq'
        ...
        loaders = Munch(src=get_test_loader(root=self.args.src_dir,
                                            img_size=self.args.img_size,
                                            batch_size=self.args.val_batch_size,
                                            shuffle=False,
                                            num_workers=self.args.num_workers),
                        ref=get_test_loader(root=self.args.ref_dir,
                                            img_size=self.args.img_size,
                                            batch_size=self.args.val_batch_size,
                                            shuffle=False,
                                            num_workers=self.args.num_workers))
